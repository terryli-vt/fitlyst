...
### Database Session验证流程

```
浏览器发请求
  → 自动带上 cookie 里的 sessionToken（一个随机字符串，比如 uuid）
  → NextAuth 拿这个 token 去 sessions 表查询
  → 找到记录 → 检查 expires 字段是否过期
  → 没过期 → 通过，用 userId 关联查 users 表拿用户信息
  → 找不到记录或已过期 → 拒绝
```

Database Session 的 cookie 里只有一个无意义的随机字符串，它本身什么信息都没有，必须去数据库查才知道对应的是哪个用户。
...
`
- 什么是JWT: JWT 是一种 token 格式，用`.`分三段：Header、Payload、Signature。Header是算法信息, Payload 存用户信息比如 user ID，Signature 是用服务端的密钥`AUTH_SECRET`对前两段做的签名，用来防止数据被篡改。
...
浏览器发请求
  → 自动带上 cookie 里的 JWT
  → NextAuth 把 JWT 拆成三段
  → 用服务器上的 AUTH_SECRET 对 header + payload 重新计算签名
  → 比较计算结果和 JWT 里的 signature 是否一致
  → 一致 → 认证通过，读取 payload 里的用户信息
  → 不一致 → 拒绝请求
```

JWT 的 payload 是 Base64 编码，任何人都能解码看到内容。但如果有人篡改了 payload（比如把 userId 改成别人的），signature 就对不上了——因为他没有你的 `AUTH_SECRET`，无法重新生成正确的签名。

所以 `AUTH_SECRET` 必须保密，它是整个 JWT 安全性的根基。

`AUTH_SECRET`是存在`.env.local`里面的环境变量，是通过跑`npx auth secret` 自动生成的那个随机字符串。NextAuth 用它来签名和验证所有 JWT。

## Session vs. Token Based Authentication

- Session Authentication（基于会话的认证）:
  - 有状态(stateful, 服务器记得你的登录信息)，登录信息存在服务端数据库，每次请求查表验证，撤销方便。
  - 可以随时强制登出: 删除数据库里的 session 记录，用户立刻失效。
  - `登录 → 服务端存 session → 把 session ID 给客户端(cookie)
请求 → 带上 cookie → 服务端查数据库验证`
  - 被盗后可以立刻撤销登录状态，安全性更高。
  - 适合传统 web 应用，或需要高安全性的应用，比如银行、企业系统等。
  - 缺点是每次请求都要查数据库，性能较差。
...
假设不用 `accounts` 表，直接在 `users` 表加字段：

```
users = {
  id, email, name,
  googleId: "google-uid-123",   // Google 登录
  githubId: "github-uid-456",   // GitHub 登录
}
```

问题一：每加一个 provider，就要改表结构

今天加 Google，加 `googleId`。明天加 GitHub，加 `githubId`。后天加 Apple，再加 `appleId`。表越来越宽，而且大部分字段对大多数用户是 NULL。

问题二：每个 provider 还要存额外数据

OAuth 不只是一个 ID，还有 `access_token`、`refresh_token`、`expires_at`、`scope`……如果都塞进 users 表，会更臃肿。
...
├─────────┼──────────┼─────────────────────┼─────────────┤
│ user_1  │ google   │ google-uid-123      │ ya29.xxx    │
│ user_1  │ github   │ github-uid-456      │ gho_xxx     │
└─────────┴──────────┴─────────────────────┴─────────────┘
```

同一个用户可以绑多个登录方式，都指向同一个 `user_1`。加新 provider 不需要改任何表结构，只是多一行数据。

这是标准的**一对多关系**设计：一个用户对应多个 OAuth 账号绑定。
...
Adapter 模式解决的问题是：**NextAuth 不知道你用什么数据库，也不想知道。**

NextAuth 内部只定义了它需要做的操作：
...
但它不知道你是用 PostgreSQL、MySQL、MongoDB 还是 SQLite，更不知道你用 Drizzle、Prisma 还是原生 SQL。

Adapter 就是一个翻译层，把 NextAuth 的抽象操作翻译成你具体数据库的实现：

```
NextAuth 说 "createUser"
  → DrizzleAdapter 翻译成 → db.insert(users).values(...)

NextAuth 说 "getUserByEmail"
  → DrizzleAdapter 翻译成 → db.select().from(users).where(eq(users.email, email))
```

你换数据库只需要换 adapter，NextAuth 的代码完全不用动。这就是 Adapter 模式的核心价值：解耦。
...
`proxy.ts` 运行在所有路由渲染之前，是请求进入应用的第一道关卡，可以在这里拦截、检查、修改或转发每一个请求。
...
Next.js 16 把 `middleware.ts` 改名为 `proxy.ts`，必须放在 `src/` 目录下。`proxy.ts` runs on the Node.js runtime.
